...
export const CHANNEL = "TECHNURTURE_HOVER_v1" as const;
const VISUAL_EDIT_MODE_KEY = "technurture_visual_edit_mode" as const;
const FOCUSED_ELEMENT_KEY = "technurture_focused_element" as const;

// Deduplicate helper for high-frequency traffic (HIT / FOCUS_MOVED / SCROLL)
// -----------------------------------------------------------------------------
let _technurtureLastMsg = "";
const postMessageDedup = (data: any) => {
  try {
    const key = JSON.stringify(data);
    if (key === _technurtureLastMsg) return; // identical – drop
    _technurtureLastMsg = key;
  } catch {
...
// Helper to parse data-technurture-id to extract file path, line, and column
const parseTechnurtureId = (
  technurtureId: string
): { filePath: string; line: number; column: number } | null => {
  // Format: "filepath:line:column"
  const parts = technurtureId.split(":");
...
export default function HoverReceiver() {
  ...
  useEffect(() => {
    if (isVisualEditMode && !styleElementRef.current) {
      ...
      style.textContent = `
        ...
        [data-technurture-protected="true"] {
          user-select: none !important;
          -webkit-user-select: none !important;
          -moz-user-select: none !important;
          -ms-user-select: none !important;
  ...
  const protectChildElements = (element: HTMLElement) => {
    ...
    childElements.forEach((child) => {
      ...
      childEl.setAttribute("data-technurture-protected", "true");
  ...
  const restoreChildElements = (element: HTMLElement) => {
    ...
    protectedElements.forEach((child) => {
      ...
      childEl.removeAttribute("data-technurture-protected");
  ...
  const handleTextChange = (element: HTMLElement) => {
    ...
    // Get the technurture ID from the element to ensure we're working with the right one
    const technurtureId = element.getAttribute("data-technurture-id");
    if (!technurtureId) return;
    ...
    if (newText !== oldText) {
      const parsed = parseTechnurtureId(technurtureId);
      if (!parsed) return;
  ...
  const handleStyleChange = (
    ...
    // Find ALL elements with the same technurture ID
    const allMatchingElements = document.querySelectorAll(
      `[data-technurture-id="${technurtureId}"]`
    ...
    const existingStyles = appliedStylesRef.current.get(technurtureId) || {};
    appliedStylesRef.current.set(technurtureId, { ...existingStyles, ...styles });
  ...
  const handleStyleBlur = (element: HTMLElement) => {
    ...
    const appliedStyles = appliedStylesRef.current.get(technurtureId);
  ...
  const flushImageSrcChange = () => {
    ...
    const technurtureId = imgElement.getAttribute("data-technurture-id");
    if (!technurtureId) return;
  ...
  useEffect(() => {
    function handleMessage(e: MessageEvent) {
      if (e.data?.type === "TECHNURTURE_STYLE_UPDATE") {
        const { elementId, styles } = e.data;

        // Find ALL elements with the same technurture ID
        const allMatchingElements = document.querySelectorAll(
          `[data-technurture-id="${elementId}"]`
        ) as NodeListOf<HTMLElement>;
      ...
      } else if (e.data?.type === "TECHNURTURE_IMAGE_UPDATE") {
  ...
  useEffect(() => {
    ...
    const handleMouseUp = () => {
      if (focusedElementRef.current && focusedElementId) {
        ...
        // Extract file info from data-technurture-id
        const technurtureId = element.getAttribute("data-technurture-id");
        if (technurtureId) {
          const parsed = parseTechnurtureId(technurtureId);
          if (parsed) {
  ...
  useEffect(() => {
    ...
    function onPointerMove(e: PointerEvent) {
      ...
      const hit =
        document
          ...
          ?.closest<HTMLElement>("[data-technurture-id]") ?? null;
      ...
      if (hit !== lastHitElementRef.current) {
        ...
        const hitId = hit.getAttribute("data-technurture-id");
        ...
        const tagName =
          hit.getAttribute("data-technurture-name") || hit.tagName.toLowerCase();

        // Update hover boxes immediately for instant feedback
        // Find ALL elements with the same technurture ID
        const allMatchingElements = document.querySelectorAll(
          `[data-technurture-id="${hitId}"]`
        ) as NodeListOf<HTMLElement>;
        ...
        allMatchingElements.forEach((element) => {
          ...
          const elementId = element.getAttribute("data-technurture-id");
    ...
    function onClickCapture(e: MouseEvent) {
      ...
      const target = e.target as HTMLElement;
      const hit = target.closest<HTMLElement>("[data-technurture-id]");
      ...
      if (hit) {
        ...
        // Find ALL other elements with the same technurture ID and show hover boxes
        const allMatchingElements = document.querySelectorAll(
          `[data-technurture-id="${hitId}"]`
        ) as NodeListOf<HTMLElement>;
        ...
        // Clicked on empty space or element without data-technurture-id
    ...
    function onMsg(e: MessageEvent<ParentToChild>) {
      ...
      if (e.data.msg === "CLEAR_INLINE_STYLES" && "elementId" in e.data) {
        // Find ALL elements with the same technurture ID
        const allMatchingElements = document.querySelectorAll(
          `[data-technurture-id="${e.data.elementId}"]`
        ) as NodeListOf<HTMLElement>;
      ...
      if (e.data.msg === "SHOW_ELEMENT_HOVER" && "elementId" in e.data) {
        ...
        if (allMatchingElements.length > 0) {
          ...
          allMatchingElements.forEach((element) => {
            ...
            if (!tagName) {
              tagName =
                element.getAttribute("data-technurture-name") ||
