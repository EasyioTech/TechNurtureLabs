...
export const students = pgTable('students', {
    ...
    first_name: text('first_name').notNull(),
    last_name: text('last_name').notNull(),
    // Schema update: Email is optional to support phone-only registration
    email: text('email'),
...
export const pinResetRequests = pgTable('pin_reset_requests', {
    ...
    // SECURITY: Removed new_pin column — PINs should never be stored in database (credential leakage risk)
    // PIN is shown once to admin in the API response and must be transmitted to student out-of-band
}, (table) => [
