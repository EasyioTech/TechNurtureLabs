...
import { 
    ...
    LogOut, Sun, Moon, Sparkles, Bell, Search, 
    ChevronRight, CreditCard, ChevronLeft
} from 'lucide-react';
...
export function SchoolDashboardLayout({ children, schoolId, schoolName: initialSchoolName }: DashboardLayoutProps) {
    ...
    const NavLink = ({ item, isMobile = false }: { item: typeof NAV_ITEMS[0], isMobile?: boolean }) => {
                ...
                {isMobileMenuOpen && (
                    <>
                        ...
                        <motion.aside
                            ...
                            className={`fixed top-0 left-0 bottom-0 w-[300px] z-[70] shadow-2xl md:hidden overflow-hidden ${ts.pageBg(isDark)}`}
                        >
                            <div className={`h-20 flex items-center justify-between px-6 border-b ${ts.border(isDark)}`}>
                                ...
                                <button onClick={() => setIsMobileMenuOpen(false)} className={`p-2 rounded-xl ${isDark ? 'bg-white/5 text-slate-400' : 'bg-slate-50 text-slate-500'}`}>
                                    <X size={20} />
                                </button>
                            </div>
                            <nav className="p-4 space-y-1.5 mt-4 overflow-y-auto max-h-[calc(100vh-160px)] custom-scrollbar">
                                {NAV_ITEMS.map(item => <NavLink key={item.id} item={item} isMobile />)}
                            </nav>
                            ...
                            <div className={`absolute bottom-0 left-0 right-0 p-6 border-t ${ts.border(isDark)} ${isDark ? 'bg-[#0f1115]' : 'bg-white'}`}>
                                <div className="flex items-center gap-3">
                                    ...
                                    <button onClick={signOut} className="p-2 text-rose-500 hover:bg-rose-500/10 rounded-lg transition-colors">
                                        <LogOut size={18} />
                                    </button>
                                </div>
                            </div>
            ...
            <main className={`flex-1 transition-all duration-500 min-w-0 ${isSidebarOpen ? 'md:pl-[280px]' : 'md:pl-[88px]'}`}>
                ...
                <header className={`sticky top-0 z-40 transition-all duration-500 ${ts.headerBg(isDark)} ${ts.border(isDark)} border-b`}>
                    <div className="h-20 px-4 md:px-8 flex items-center justify-between gap-4 max-w-[1600px] mx-auto">
                        ...
                        {/* Middle Search (Professional Feel) */}
                        <div className="hidden lg:flex items-center flex-1 max-w-md relative group px-2">
                            ...
                            <input 
                                ...
                                className={`w-full h-11 pl-12 pr-4 rounded-xl text-[13px] font-medium border transition-all ${isDark ? 'bg-white/[0.03] border-white/[0.05] focus:bg-white/[0.05] focus:border-indigo-500/30' : 'bg-slate-50 border-slate-200/60 focus:bg-white focus:border-indigo-200'} focus:outline-none placeholder:text-slate-500`}
                            />
                        </div>
                        ...
                        <div className="flex items-center gap-2 sm:gap-4 ml-auto">
                            <div className="flex items-center gap-1 sm:gap-2">
                                ...
                                <button className={`w-10 h-10 rounded-xl flex items-center justify-center relative transition-all ${isDark ? 'text-slate-400 hover:bg-white/5' : 'text-slate-500 hover:bg-slate-100'}`}>
                                    <Bell size={18} />
                ...
                {/* Content Area */}
                <div className="p-4 md:p-8 lg:p-10 max-w-[1700px] mx-auto min-h-[calc(100vh-80px)]">
