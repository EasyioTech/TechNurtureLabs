...
import { HardDrive, Download, RotateCcw, AlertCircle, CheckCircle2, Info, RefreshCw, ChevronLeft, Loader2 } from 'lucide-react';
    ...
    return (
        <div className={`min-h-screen ${isDark ? 'bg-neutral-900' : 'bg-slate-50'}`}>
            ...
            <div className="max-w-5xl mx-auto px-4 sm:px-6 py-8">
                        ...
                        {backups.map((backup, idx) => (
                            ...
                            >
                                <div className="flex-1 min-w-0">
                                    <p className={`text-sm font-semibold break-all ${t.textPrimary(isDark)}`}>
                                        {backup.fileName}
                                    </p>
                ...
                {/* Info Modal */}
                {showInfo && selectedBackup && (
                    <div className={`fixed inset-0 z-50 flex items-center justify-center p-4 ${isDark ? 'bg-black/50' : 'bg-black/30'}`}>
                        <div className={`rounded-lg p-6 max-w-sm w-full border ${isDark ? 'bg-neutral-800 border-white/10' : 'bg-white border-slate-200'}`}>
                            <h3 className={`text-lg font-black mb-4 ${t.textPrimary(isDark)}`}>Backup Details</h3>
                            <div className="space-y-3 mb-6">
                                <div>
                                    <p className={`text-xs font-black uppercase ${t.textMuted(isDark)} mb-1`}>File</p>
